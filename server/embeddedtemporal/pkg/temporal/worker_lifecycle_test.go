...
func TestWorkerLifecycle_SimpleWorkflow_FastTeardown(t *testing.T) {
	...
	require.Eventually(t, func() bool {
		...
		defer cancel()
		_, err := cli.WorkflowService().DescribeNamespace(ctx, &workflowservice.DescribeNamespaceRequest{Namespace: "client-test"})
		return err == nil
	}, 30*time.Second, 200*time.Millisecond, "namespace client-test not ready")

	// Allow namespace cache to propagate across services before running workflows.
