...
func TestHistoryWithWorkflowTestSuite(t *testing.T) {
	...
	for typ, reg := range registry.GetAll() {
		...
		// Capture reg for closure
		activityImpl := reg.Activity
		env.RegisterActivityWithOptions(func(ctx context.Context, req workerops.ActivityInvocationRequest) (map[string]interface{}, error) {
			input := req.Input
			if input == nil {
	...
	if err != nil {
		t.Fatalf("transform history failed: %v", err)
	}

	// Validate: two parent command executions + one child command execution
	if len(acts) != 3 {
		t.Fatalf("expected 3 activities (including child), got %d", len(acts))
	}
	found := map[string]bool{"parent-hello": false, "child": false, "done": false}
	for i, a := range acts {
		if a.Name != "command_execution" {
			t.Fatalf("activity %d name: %s", i, a.Name)
		}
		if a.Status != "completed" {
			t.Fatalf("activity %d status: %s", i, a.Status)
		}
		if a.Duration == nil {
			t.Fatalf("activity %d missing duration", i)
		}
		if a.Result == nil {
			t.Fatalf("activity %d missing result", i)
		}
	...
	for k, v := range found {
		if !v {
			t.Fatalf("missing stdout %q in activities", k)
		}
	}
	...
	if err != nil {
		t.Fatalf("execution to job failed: %v", err)
	}
	if job.Status != recipecore.JobStatusCompleted {
