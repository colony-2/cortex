...
type mockFileAdapter struct {
	...
	generateWithFiles func(context.Context, string, []File, Config) (Response, error)
...
func (m *mockFileAdapter) GenerateWithFiles(ctx context.Context, prompt string, files []File, config Config) (Response, error) {
...
func (m *mockFileAdapter) ValidateFile(file File) error {
...
func TestGetFileType(t *testing.T) {
	tests := []struct {
		...
		{"text file", "text/plain", FileTypeText},
		{"javascript", "application/javascript", FileTypeText},
		{"json", "application/json", FileTypeText},
		{"jpeg image", "image/jpeg", FileTypeImage},
		{"png image", "image/png", FileTypeImage},
		{"pdf", "application/pdf", FileTypePDF},
		{"mp3 audio", "audio/mp3", FileTypeAudio},
		{"mp4 video", "video/mp4", FileTypeVideo},
		{"unknown", "application/octet-stream", FileTypeUnknown},
	}
...
func TestOpenAIAdapter_FileHandling(t *testing.T) {
	...
	t.Run("capabilities", func(t *testing.T) {
		...
		supportedTypes := map[FileType]bool{
			FileTypeText:  false,
			FileTypeImage: false,
		}
		...
		if !supportedTypes[FileTypeText] {
			t.Error("OpenAI should support text files")
		}
		if !supportedTypes[FileTypeImage] {
...
func TestGeminiAdapter_FileHandling(t *testing.T) {
	...
	t.Run("capabilities", func(t *testing.T) {
		...
		expectedTypes := []FileType{
			FileTypeText,
			FileTypeImage, // Limited support
			FileTypePDF,   // Limited support
		}
		
		supportedTypes := make(map[FileType]bool)
		for _, typ := range caps.SupportedTypes {
			supportedTypes[typ] = true
		}
...
func TestFileAdapter_GenerateWithFiles(t *testing.T) {
	...
	t.Run("text files only", func(t *testing.T) {
		adapter := &mockFileAdapter{
			generateWithFiles: func(ctx context.Context, prompt string, files []File, config Config) (Response, error) {
			...
			capabilities: FileCapabilities{
				SupportedTypes: []FileType{FileTypeText},
	...
	t.Run("mixed file types", func(t *testing.T) {
		adapter := &mockFileAdapter{
			...
			capabilities: FileCapabilities{
				SupportedTypes: []FileType{FileTypeText, FileTypeImage, FileTypePDF},
