...
type dummyIn struct{ A string `json:"a"` }
type dummyOut struct{ B string `json:"b"` }

func TestOps_Register_Get_List_Clear(t *testing.T) {
    Clear()
    // Operations register globally for workflow access [pkg/ops/ops.go]
    op := NewActivityMappedOp(OpMetadata{Type: "t1", Name: "op1"}, func(ctx context.Context, in dummyIn) (dummyOut, error) {
    ...
    got, ok := Get("op1")
    assert.True(t, ok)
    assert.Equal(t, "op1", got.GetName())

    // Duplicate operation names replace existing registrations [pkg/ops/ops.go]
    op2 := NewActivityMappedOp(OpMetadata{Type: "t1", Name: "op1"}, func(ctx context.Context, in dummyIn) (dummyOut, error) {
...
func TestOps_ThreadSafety(t *testing.T) {
    ...
    for i := 0; i < 100; i++ {
        ...
        go func() {
            ...
            Register(NewActivityMappedOp(OpMetadata{Type: name, Name: name}, func(ctx context.Context, in dummyIn) (dummyOut, error) {
                return dummyOut{B: in.A}, nil
            }))
        }()
    }
