...
type rIn struct{ Msg string `json:"msg"` }
type rOut struct{ Echo string `json:"echo"` }
...
func TestRegisterableOp_Activity_And_Inline(t *testing.T) {
    ...
    inline := NewInlineOp[rIn, rOut](OpMetadata{Name: "i1"}, func(ctx workflow.Context, timeout time.Duration, retry *temporal.RetryPolicy, in rIn) (rOut, error) {
    ...
    act := NewActivityMappedOp[rIn, rOut](OpMetadata{Name: "a1"}, func(ctx context.Context, in rIn) (rOut, error) {
        return rOut{Echo: in.Msg}, nil
    })
...
func TestRegisterableOp_ErrorAndPanics(t *testing.T) {
    ...
    act := NewActivityMappedOp[rIn, rOut](OpMetadata{Name: "a2"}, func(ctx context.Context, in rIn) (rOut, error) { return rOut{}, nil })
    ...
    inlineOnly := NewInlineOp[rIn, rOut](OpMetadata{Name: "inline-only"}, func(ctx workflow.Context, timeout time.Duration, retry *temporal.RetryPolicy, in rIn) (rOut, error) { return rOut{}, nil })
    ...
    actOnly := NewActivityMappedOp[rIn, rOut](OpMetadata{Name: "act-only"}, func(ctx context.Context, in rIn) (rOut, error) { return rOut{}, nil })
