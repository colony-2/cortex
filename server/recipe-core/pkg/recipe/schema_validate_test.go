...
type schemaEchoIn struct{ Msg string `json:"msg" yaml:"msg"` }
type schemaEchoOut struct{ Out string `json:"out" yaml:"out"` }
...
func TestSchema_Generate_With_Inline_Op(t *testing.T) {
    ops.Clear()
    type in struct{ A string `json:"a" yaml:"a"` }
    type out struct{ B string `json:"b" yaml:"b"` }
    // Register an inline op and ensure schema generation does not panic and includes it
    inline := ops.NewInlineOp[in, out](
        ops.OpMetadata{Type: "recipe"}, // inline op type
        func(ctx workflow.Context, timeout time.Duration, retry *temporal.RetryPolicy, i in) (out, error) {
            _ = ctx
            _ = timeout
            _ = retry
