# Bug: `JobFailedError` / `GetJobRun(...).GetOutput(...)` path panicked with `hash of unhashable type swf.AppError`

## Status

Resolved on `swf-go` HEAD `6215190fc4c615f2775980856377b3c241c81a08` (`v0.0.0-20260318205952-6215190fc4c6`).

Retested downstream on both `runtime/toy` and `runtime/direct` with the original parent/child reproducer and the broader `state-child-run` failure cases. Those now return the expected child failure (`job failed: ... exit status ...`) instead of panicking with `hash of unhashable type swf.AppError`.

## Summary

On `swf-go` HEAD `96cd1785f854fa05c99b8ffe14c044bd2fe15cc4` (`v0.0.0-20260318201836-96cd1785f854`), a parent workflow that resolves a failed child through `GetJobRun(...).GetOutput(...)` failed with:
