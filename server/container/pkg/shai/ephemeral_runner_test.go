...
func TestEphemeralContainerSetup(t *testing.T) {
	tests := []struct {
		...
		{
			name: "bootstrap runs during postcreate even without user command",
			dc: &devcontainer.DevContainer{
				ImageContainer: &devcontainer.ImageContainer{
					Image: "ubuntu:22.04",
				},
			...
			aliasEnv: map[string]string{
				"ALLOW_DOCKER_HOST_PORT":  "7777",
				"SHAI_ALIAS_SSH_HOSTPORT": "host.docker.internal:7777",
			},
			validate: func(t *testing.T, script string) {
				assert.Contains(t, script, "::DEVCONTAINER::POSTCREATE::START::")
				assert.Contains(t, script, "export ALLOW_DOCKER_HOST_PORT='7777'")
				assert.Contains(t, script, "export SHAI_ALIAS_SSH_HOSTPORT='host.docker.internal:7777'")
				assert.Contains(t, script, "bootstrap_cmd=\"\"")
				assert.Contains(t, script, "echo \"[bootstrap] running $bootstrap_cmd (pid $$)\"")
				assert.Contains(t, script, "\"[bootstrap] started at $(date -Iseconds)\"")
				assert.Contains(t, script, "/usr/local/sbin/bootstrap.sh")
			},
		},
	}
