...
func init() {
	...
	StartCmd.Flags().Bool("in-memory", false, "Use in-memory database instead of persistent storage")
...
func runDevServer(cmd *cobra.Command, args []string) error {
	// Check if in-memory flag is set
	...
	// Ensure absolute path for database file if not in-memory
	if !opts.InMemory && opts.DatabaseFile != "" {
