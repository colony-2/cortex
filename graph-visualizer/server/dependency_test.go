...
func TestUpdateDependenciesHandler(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:         "Update existing node dependencies",
			nodeID:       "nodeA",
			dependencies: []string{"nodeB", "nodeC"},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			req := httptest.NewRequest("PUT", "/api/nodes/"+tt.nodeID+"/dependencies", bytes.NewReader(body))
			...
			// Call the handler
			updateDependenciesHandler(rr, req)
			...
			if tt.wantStatus == http.StatusOK {
				...
				if len(dep.Dependencies) != len(tt.dependencies) {
					t.Errorf("Dependencies count mismatch: got %d want %d",
						len(dep.Dependencies), len(tt.dependencies))
				}
				
				for i, d := range dep.Dependencies {
					if i < len(tt.dependencies) && d != tt.dependencies[i] {
						t.Errorf("Dependency mismatch at index %d: got %s want %s",
							i, d, tt.dependencies[i])
					}
				}
			}
		})
...
func TestCircularDependencyPrevention(t *testing.T) {
	...
	// - C from depending on A (would create cycle: A->B->C->A)
	// - C from depending on B (would create cycle: B->C->B)
	// - B from depending on A (would create cycle: A->B->A)
	
	// But these should be allowed:
	// - A can add C as dependency (no cycle: A->B,C and B->C)
	// - A new node D could depend on any of A, B, C
}