...
- `.colony2/ops/<op_name>/` must contain:
  - `op.yaml` or `op.yml` (required): Defines metadata, command, and optional settings.
...
- Minimal, pragmatic spec (all fields optional unless marked required):
  ...
  - Schemas (optional, JSON Schema Draft 2020):
    - `input_schema` (object): Used for `cortex schema`, YAML parse-time validation, and runtime input validation.
    - `output_schema` (object): Used for documentation and runtime output validation (if provided).
...
- Input/output schemas are validated when provided:
  - `input_schema` is enforced both when parsing recipe YAML and again before execution.
  - `output_schema` is enforced after execution (stdout JSON).
- If `input_schema` is omitted, inputs are treated as a permissive object (any keys allowed).
