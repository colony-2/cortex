...
test.describe('URL State Management', () => {
  ...
  test('should update URL when selecting a cell', async ({ page }) => {
    await page.goto('/cells');
    
    // Wait for graph to load and cells to be rendered
    ...
    if (count === 0) {
      ...
      cellId = await graphCell.getAttribute('data-cell-id');
    }
    
    await graphCell.click({ force: true });
    ...
    // Check URL contains cell in path
    await expect(page).toHaveURL(new RegExp(`/cell/${cellId}/files`));
  });
  ...
  test('should update URL when changing tabs', async ({ page }) => {
    ...
    await expect(page).toHaveURL(new RegExp(`/cell/${cellId}/changes`));
  });
  ...
  test('should update URL when changing subtabs in Changes', async ({ page }) => {
    ...
    await expect(page).toHaveURL(new RegExp(`/cell/${cellId}/changes/history`));
  });
  ...
  test('should restore state from URL on page load', async ({ page }) => {
    ...
    const graphCell = node.locator('.graph-cell');
    const cellId = await graphCell.getAttribute('data-cell-id');
    
    // Navigate directly with path
    await page.goto(`/cell/${cellId}/changes/history`);
    ...
    expect(page.url()).toContain(`/cell/${cellId}/changes/history`);
