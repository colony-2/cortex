#!/bin/bash
# test_runner.sh - Test runner for the intercept shim
...
DYLD_INSERT_LIBRARIES=./intercept.dylib ./test_app
TEST_EXIT=$?
echo "Exit code: $TEST_EXIT"

kill $MONITOR_PID 2>/dev/null || true
wait $MONITOR_PID 2>/dev/null || true
rm -f test_output.txt /tmp/colony2-rwshim.sock
...
DYLD_INSERT_LIBRARIES=./intercept.dylib ./test_app 2>&1 || echo "Expected failure - Exit code: $?"
